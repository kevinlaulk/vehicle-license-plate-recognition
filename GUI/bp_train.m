...
option_function=2;%功能选择：1-统计；2-结构
switch option_function
    case 1
        p=zeros(N*M,size_x+size_y);
        t=zeros(M,N*M);
    case 2
        p=zeros(N*M,43);
        t=zeros(M,N*M);
...
for n=1:N
    for m=1:M
        if m<=10%数字
        file_name=strcat('..\数字模板\',bp_liccode(m),'\',int2str(n),'.bmp'); %读取数字模板
        else 
        file_name=strcat('..\英文模板\',bp_liccode(m),'\',int2str(n),'.bmp'); %读取英文模板
        end
        sample=imresize(imread(file_name),[size_x size_y],'bilinear');%大小重塑 双线性插值
        sample=im2bw(sample);%二值化,模板原图
%         figure,imshow(sample);title('模板原图')
        switch tezheng
        case 1
        %====统计特征提取====
        histcol=sum(sample); %计算垂直投影
        histrow=sum(sample'); %计算水平投影
        p(m+n-1,:)=[histcol,histrow];
        t(m,m+n-1)=1;
        case 2
        %===结构特征提取====
...
%         end
        ...
        %=========特征提取结束=========
        p(m+n-1,:)=[left_col,mid_col,right_col,left_row,mid_row,right_row,reshape(rec_1,[1,6]),reshape(rec_2,[1,6]),reshape(rec_3,[1,6]),reshape(rec_4,[1,6]),count_bihe,Area,...
...
%=========特征提取结束==========
%==========创建网络/训练网络============
net=newff(minmax(p),[43,40,36],{'logsig','logsig','purelin'},'trainrp');%新建网络   隐藏层节点数sqrt(输入节点*（输出节点+1）+1) 可增加以提高收缩速率
